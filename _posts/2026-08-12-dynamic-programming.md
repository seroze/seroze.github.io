...
*A running collection of DP patterns — one section per trick, added to over time.*

---

## Contents {#contents}

1. [DP on residues](#residue-dp) — an unbounded running sum collapses to $$m$$ buckets when the constraint only looks at the remainder. *(CodeChef MULT9)*
1. [Push, don't pull](#push-vs-pull) — write transitions as `dp[i+1] += dp[i]` so you never have to invert them.

---

## DP on residues {#residue-dp}
...
## Push, don't pull {#push-vs-pull}

There are two ways to write any DP transition.
...
| Style | Form | Verdict |
|---|---|---|
| Push | `dp[i+1][(r + d) % m] += dp[i][r]` | default — follows the transition |
| Pull | `dp[i+1][r] += dp[i][(r - d) % m]` | avoid — needs the inverse |
